...
namespace TrailDLL
{
	public class TrailStretchedBillboard : TrailStretched
	{
        public TrailStretchedBillboard(Game hGame, int iLength, float fRadius, Color vStartColor, Color vEndColor, BlendState vBlend, bool bShrink, Texture2D hTexture, float fTextureRepeat, SamplerState hSamplerState)
        ...
        internal override void InnerUpdate()
        {
			...
			m_vVertexData[iIndex + 1].TextureCoordinate.X = 0;
			...
			for (int i = 1; i < m_hSegmentStack.Count; i++)
			{
				...
				if (m_bColored)
				{
					...
					m_vVertexData[iIndex + 1].Color = m_vVertexData[iIndex].Color = m_vStartColor;
				...
				//fWidth = hCurrentSegment.Radius * 2;
				vRight = new Vector3(
                  vBillboard.M11 * hCurrentSegment.Radius,
                  vBillboard.M12 * hCurrentSegment.Radius,
                  vBillboard.M13 * hCurrentSegment.Radius
                ...
                //float fStep = (i + 1) * (1f / m_hSegmentStack.Count);//m_fStep * 2 * (i - 1);
